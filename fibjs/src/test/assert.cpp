    ...
    if (!IsEmpty(actual) && !IsEmpty(expected) && !actual->IsFunction()
        ...
        if (actual->IsDate())
            return expected->IsDate()
                && (actual->NumberValue(context).ToChecked() == expected->NumberValue(context).ToChecked());
    ...
    v8::Local<v8::Value> value, bool& retVal, v8::Local<v8::Value>& got)
{
    ...
    v8::Local<v8::Object> obj = isolate->toLocalObject(object);
    got = obj->Get(prop);
    if (got.IsEmpty())
        return CALL_E_JAVASCRIPT;

    retVal = isolate->isEquals(value, got, context);
