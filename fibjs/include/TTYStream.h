...
static const char* kClearToLineBeginning = "\x1b[1K";
static const char* kClearToLineEnd = "\x1b[0K";
static const char* kClearLine = "\x1b[2K";
static const char* kClearScreenDown = "\x1b[0J";

class TTYInputStream : public UVStream_tmpl<TTYInputStream_base> {
public:
    TTYInputStream()
        : UVStream_tmpl<TTYInputStream_base>(_fileno(stdin))
        , m_isRaw(false)
    {
        uv_call([&] {
            uv_tty_init(s_uv_loop, &m_tty, _fileno(stdin), 0);
