...
/**
 * @description 图像处理对象，用以对图像进行转换，绘制，存储等操作
 * 
 *  Image 对象属于 gd 模块，创建：
 *  ```JavaScript
 ...
 *  
 */
declare class Class_Image extends Class_object {
    /**
     * @description 查询图像宽度 
     */
    ...
    /**
     * @description 查询图像来源格式，结果为 gd.PNG, gd.JPEG, gd.GIF, gd.BMP, gd.WEBP 
     */
    readonly format: number;

    /**
     * @description 查询图像类型，结果为 gd.TRUECOLOR, gd.PALETTE 
     */
    readonly type: number;

    /**
     * @description 查询图像颜色表内的颜色总数 
     */
    readonly colorsTotal: number;

    /**
     * @description 查询和设定指定的颜色为透明色 
     */
    transparent: number;

    /**
     * @description 查询和设定图像是否渐进式，仅支持 jpeg 格式时 
     */
    progressive: boolean;

    /**
     * @description查询和设定绘图时是否计算 alpha 层，缺省为 true 
     */
    alphaBlending: boolean;

    /**
     * @description 按照指定的格式返回图像数据
