...
/**
	* @brief 文件系统处理模块
	* @detail 使用方法：,```JavaScript,var fs = require('fs');,```,,一些注意点:,,- 运行 `fs.watch(filename)` 会返回一个继承自 EventEmitter 的 watcher, 它支持 'change', 'changeonly', 'renameonly' 三个事件,- `fs.watchFile(target)` 和 `fs.unwatchFile(target)` 依然可以成对使用,- `fs.watchFile(target)` 会返回一个继承自 EventEmitter 的 StatsWatcher 对象, 调用 `fs.unwatchFile(target)` 等价于调用 `StatsWatcher.close()`.
	*/
declare module "fs" {
	 ...
	 * 
	 */
	export function watch(fname: string): Class_FSWatcher;
	 ...
	 * 
	 */
	export function watch(fname: string, callback: Function): Class_FSWatcher;
	 ...
	 * 
	 */
	export function watch(fname: string, options: Fibjs.AnyObject): Class_FSWatcher;
	 ...
	 * 
	 */
	export function watch(fname: string, options: Fibjs.AnyObject, callback: Function): Class_FSWatcher;
	 ...
	 * 
	 */
	export function watchFile(fname: string, callback: Function): Class_StatsWatcher;
	 ...
	 * 
	 */
	export function watchFile(fname: string, options: Fibjs.AnyObject, callback: Function): Class_StatsWatcher;
	 ...
	 * 
	 */
	export function unwatchFile(fname: string): void;
	 ...
	 * 
	 */
	export function unwatchFile(fname: string, callback: Function): void;
